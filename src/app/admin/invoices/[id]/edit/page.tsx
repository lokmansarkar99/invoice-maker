  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setError(json.message || "Failed to edit invoice");
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4 mb-8">
        <Link href={`/admin/invoices/${id}`} className="p-2 bg-white border border-gray-200 hover:bg-gray-50 rounded-lg transition-colors text-gray-700 hover:text-gray-900 shadow-sm">
          <ArrowLeft size={20} />
        </Link>
                ...
                {items.length === 0 ? (
                  ...
                  <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">
                      ...
                      {item.stock !== '?' && <div className={`text-xs ${item.quantity > item.stock ? "text-red-600" : "text-gray-500"}`}>Stock: {item.stock}</div>}
                    </td>
                    <td className="py-3 px-4">
                      <input 
                        type="number" 
                    ...
                    <td className="py-3 px-4">
                      <input 
                        ...
                        className="w-24 bg-white border border-blue-600 rounded-lg px-2 py-1 text-gray-900 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600"
                      />
                    </td>

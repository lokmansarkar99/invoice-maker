          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10">
                <th className="py-3 px-4 text-gray-400 font-medium">Invoice ID</th>
              ...
              {loading ? (
                ...
                invoices.map((invoice) => (
                  <tr key={invoice._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="py-3 px-4 font-medium text-teal-400">{invoice.invoiceNumber}</td>
                    <td className="py-3 px-4 text-white">
                      <div>{invoice.customer.name}</div>
                      <div className="text-xs text-gray-400">{invoice.customer.phone}</div>
                    </td>
                    <td className="py-3 px-4 text-gray-300">৳{invoice.grandTotal.toLocaleString()}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        invoice.status === 'PAID' ? 'bg-emerald-500/20 text-emerald-400' :
                        invoice.status === 'DUE' ? 'bg-orange-500/20 text-orange-400' :

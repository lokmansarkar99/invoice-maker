...
export default function ProductsPage() {
  ...
  return (
    <div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-cyan-900 light:border-slate-300 bg-cyan-950/30 light:bg-white">
                <th className="py-3 px-4 text-cyan-600 light:text-slate-700 font-bold text-xs tracking-widest light:tracking-normal">Product Name</th>
                ...
                <th className="py-3 px-4 text-cyan-600 light:text-slate-700 font-bold text-xs tracking-widest light:tracking-normal">Stock</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={4} className="py-8 text-center text-cyan-700 light:text-slate-600 animate-pulse tracking-widest light:tracking-normal text-xs font-bold">Loading...</td></tr>
              ) : products.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-red-500 tracking-widest light:tracking-normal text-xs font-bold">
                ...
                products.map((product) => (
                  <tr key={product._id} className="border-b border-cyan-900/50 hover:bg-cyan-950/30 transition-colors group">
                    <td className="py-3 px-4 font-medium text-cyan-400 light:text-slate-900">{product.name}</td>
                    <td className="py-3 px-4 text-cyan-300 light:text-slate-900">৳{product.standardPrice.toLocaleString()}</td>
                    <td className="py-3 px-4 text-cyan-300 light:text-slate-900">{product.stock}</td>
      ...
      {/* Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 light:bg-white backdrop-blur-[2px] z-50 flex items-center justify-center p-4">
          <div className="bg-black/95 light:bg-white border border-cyan-500/80 shadow-[0_0_40px_rgba(6,182,212,0.3)] w-full max-w-md overflow-hidden relative">
            <div className="absolute top-0 left-0 right-0 h-1 bg-cyan-400"></div>
            <div className="flex justify-between items-center p-6 border-b border-cyan-900 light:border-slate-300">
              <h2 className="text-lg font-bold text-cyan-400 light:text-slate-900 tracking-widest light:tracking-normal inline-flex items-center gap-2">
                <Terminal size={18} />
                {editingId ? "EDIT_NODE_DATA" : "INITIALIZE_NODE"}
              </h2>
              <button onClick={handleCloseModal} className="text-cyan-600 light:text-slate-700 hover:text-cyan-400 light:hover:text-indigo-600 transition-colors">
                <X size={24} />
              </button>
            </div>
            
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              {modalError && <div className="p-3 bg-red-950/50 light:bg-red-50 text-red-500 border border-red-500/50 light:border-red-200 text-xs font-bold tracking-widest light:tracking-normal">{modalError}</div>}
              
              <div className="group">
                <label className="text-[10px] font-bold text-cyan-600 light:text-slate-700 tracking-widest light:tracking-normal block mb-1">
                  // PRODUCT_IDENTIFIER *
                </label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-black light:bg-white border border-cyan-900 light:border-slate-300 text-cyan-300 light:text-slate-900 pl-8 pr-4 py-2 focus:outline-none focus:border-cyan-400 light:focus:border-indigo-400 focus:shadow-[0_0_15px_rgba(6,182,212,0.2)] transition-colors" 
                  />
                </div>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-cyan-900 light:border-slate-300 mt-6">
                <button 
                  type="button" 
                  onClick={handleCloseModal}
                  className="px-4 py-2 text-cyan-700 light:text-slate-600 hover:text-cyan-400 light:hover:text-indigo-600 border border-transparent hover:border-cyan-900 transition-colors font-bold text-xs tracking-widest light:tracking-normal"
                >
                  ABORT
                </button>
                <button 
                  ...
                  className="bg-cyan-950/50 light:bg-slate-200 disabled:opacity-50 text-cyan-400 light:text-slate-900 font-bold py-2 px-6 border border-cyan-500/50 light:border-slate-300 hover:bg-cyan-900/50 light:hover:bg-indigo-50 hover:text-cyan-300 light:hover:text-indigo-700 hover:shadow-[0_0_15px_rgba(6,182,212,0.4)] transition-all text-xs tracking-widest light:tracking-normal"
                >
